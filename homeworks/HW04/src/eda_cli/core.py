...
def compute_quality_flags(summary: DatasetSummary, missing_df: pd.DataFrame) -> Dict[str, Any]:
    ...
    - есть дубликаты в признаке-идентификаторе
    - слишком много категорий в сравнении с количеством строк
    - есть ли колонки со всеми одинаковыми значениями
    """
    flags["has_id_duplicates"] = summary.columns[0].non_null != summary.columns[0].unique

    cardinality_threshold = round(summary.n_rows * 0.33)
    summary_df = flatten_summary_for_print(summary)
    non_numeric_cols_df = summary_df[ (summary_df["dtype"] == "object") | 
                                      (~summary_df["is_numeric"]) ]
    flags["has_high_cardinality_categoricals"] = ((non_numeric_cols_df["unique"] > cardinality_threshold) \
                                                 & (non_numeric_cols_df["unique"] < summary.n_rows)).any()
    
    flags["has_constant_columns"] = (summary_df["unique"] == 1).any()
